...
const purchaseProductSlice = createSlice({
  ...
  reducers: {
    ...
    updateProductCartStatus: (state, action: PayloadAction<{ productId: number; addToCart: boolean }>) => {
    ...
    updateProductRating: (state, action: PayloadAction<{ productId: number; rating: number; reviews?: number }>) => {
...
export const selectProductsByCategory = (
  ...
  return state.purchaseProducts.products.filter(product => product.category === category);
};
...
export const selectInStockProducts = (state: { purchaseProducts: PurchasedProductState }) =>
  state.purchaseProducts.products.filter(product => product.inStock !== false);

// Selector for products that can be added to cart
export const selectCartableProducts = (state: { purchaseProducts: PurchasedProductState }) =>
  state.purchaseProducts.products.filter(product => product.inStock !== false && product.addToCart !== false);

// Selector for products with ratings
export const selectRatedProducts = (state: { purchaseProducts: PurchasedProductState }) =>
  state.purchaseProducts.products.filter(product => product.rating !== undefined && product.rating > 0);
