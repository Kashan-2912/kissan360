...
import type { PurchaseProduct } from '../types/index';
  ...
  reducers: {
    addToCart: (state, action: PayloadAction<{ product: PurchaseProduct; quantity?: number }>) => {
      ...
      if (existingItem) {
        ...
        existingItem.totalPrice = existingItem.quantity * parsePriceString(product.price);
      } else {
        // Add new item
    ...
    updateQuantity: (state, action: PayloadAction<{ cartItemId: number; quantity: number }>) => {
      ...
      if (item && quantity > 0) {
        ...
        state.items = state.items.filter(cartItem => cartItem.id !== cartItemId);
...
export const selectCartItemByProductId = (state: { cart: CartState }, productId: number) =>
  state.cart.items.find(item => item.product.id === productId);

export const selectCartItemById = (state: { cart: CartState }, cartItemId: number) =>
  state.cart.items.find(item => item.id === cartItemId);

export const selectIsProductInCart = (state: { cart: CartState }, productId: number) =>
